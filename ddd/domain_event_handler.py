...
from .instance_getter import get_instance

_domain_event_handlers = defaultdict(list)

_DomainEvent = TypeVar('_DomainEvent')
...
class domain_event_handler:
    ...
    def __init__(self, method):
        ...
        domain_event_class: Type[_DomainEvent] = func_spec.annotations[func_spec.args[1]]
...
def handle_domain_event(domain_event: object) -> None:
    ...
    if domain_event_class in _domain_event_handlers:
        for handler_cls, handler in _domain_event_handlers[domain_event_class]:
            handler_instance = get_instance(handler_cls)
